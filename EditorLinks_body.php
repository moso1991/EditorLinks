...
class EditorLinks extends SpecialPage {
	/**
	 * Constructor
	 */
	function __construct() {
		parent::__construct( 'EditorLinks' );
	}
	...
	function createInitialTree() {
		...
		// 'general' section
		$general_section = new ALSection( $this->msg( 'editorlinks_general' )->text() );
		...
		$main_row->addItem( ALItem::newFromSpecialPage( 'Specialpages' ) );
                 $main_row->addItem( ALItem::newFromSpecialPage( 'SecurePoll' ) );
		$main_row->addItem( ALItem::newFromSpecialPage( 'Log' ) );
$main_row->addItem( ALItem::newFromSpecialPage( 'ApiSandbox' ) );
		$main_row->addItem( ALItem::newFromSpecialPage( 'Allmessages' ) );
$main_row->addItem( ALItem::newFromSpecialPage( 'CategoryTree' ) );
$main_row->addItem( ALItem::newFromSpecialPage( 'Drafts' ) );
		$main_row->addItem( ALItem::newFromEditLink(
			'Sidebar',
			$this->msg( 'editorlinks_editsidebar' )->text()
		) );
		$main_row->addItem( ALItem::newFromEditLink(
			'Common.css',
			$this->msg( 'editorlinks_editcss' )->text()
		) );
		$main_row->addItem( ALItem::newFromEditLink(
			'Mainpage',
		...
		// 'users' section
		$users_section = new ALSection( $this->msg( 'editorlinks_users' )->text() );
		$main_row = new ALRow( 'main' );
		$main_row->addItem( ALItem::newFromSpecialPage( 'Listusers' ) );
		$ul = SpecialPage::getTitleFor( 'Userlogin' );
		$al = SpecialPage::getTitleFor( 'Editorlinks' );

		$main_row->addItem( AlItem::newFromPage( $ul, $this->msg( 'editorlinks_createuser' )->text(),
		...
		$browse_search_section = new ALSection( $this->msg( 'editorlinks_browsesearch' )->text() );
		$main_row = new ALRow( 'main' );
	...
	$main_row->addItem( ALItem::newFromSpecialPage( 'ProtectedPages' ) );
		$browse_search_section->addRow( $main_row );
		$tree->addSection( $browse_search_section );

		// 'importing and exporting' section
		$import_export_section = new ALSection( $this->msg( 'editorlinks_importexport' )->text() );
		$main_row = new ALRow( 'main' );
	...
	function execute( $query ) {
		$this->setHeaders();
		$editor_links_tree = $this->createInitialTree();
		Hooks::run( 'Editorlinks', array( &$editor_links_tree ) );
		global $wgOut;
		if ( method_exists( $wgOut, 'addModuleStyles' ) &&
			!is_null( $wgOut->getResourceLoader()->getModule( 'mediawiki.special' ) ) ) {
			$wgOut->addModuleStyles( 'mediawiki.special' );
		}
		$wgOut->addHTML( $editor_links_tree->toString() );
	}

	/**
	 * For administrators, add a link to the special 'Editorlinks' page
	 * among the user's "personal URLs" at the top, if they have
	 * the 'Editorlinks' permission.
	 *
	...
	public static function addURLToUserLinks( array &$personal_urls, Title &$title ) {
		...
		if ( $wgUser->isAllowed( 'editorlinks' ) ) {
			$al = SpecialPage::getTitleFor( 'Editorlinks' );
			$href = $al->getLocalURL();
			$editor_links_vals = array(
				'text' => wfMessage( 'editorlinks' )->text(),
				'href' => $href,
				'active' => ( $href == $title->getLocalURL() )
			);

			// find the location of the 'my preferences' link, and
			// add the link to 'editorlinks' right before it.
			...
			array_splice( $tab_keys, $prefs_location, 0, 'editorlinks' );
			array_splice( $tab_values, $prefs_location, 0, array( $editor_links_vals ) );
...
/**
 * The 'tree' that holds all the sections, rows, and links for the editorlinks
 * page
 */
class ALTree {
	public $sections;
...
/**
 * A single row of the editorlinks page, with a name (not displayed, used only
...
/**
 * A single 'item' in the editorlinks page, most likely representing a link
	...
	static function newFromSpecialPage( $page_name ) {
		...
		if ( $page ) {
			...
			$wgOut->addHTML( "<span class=\"error\">" .
			    wfMessage( 'editorlinks_pagenotfound', $page_name )->escaped() . "<br></span>" );
		}
		return $item;
	}
